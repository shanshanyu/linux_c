...
int register_client(my_pipe_t *ptr,int opmap){ //注册读写管道的客户端，客户端类型通过 opmap 参数（位图）传递
...
int unregister_client(my_pipe_t *ptr,int opmap){
  //注销客户端
  struct my_pipe_st *p = ptr;
  if(pthread_mutex_lock(&p->mut) != 0)
    return -1;
...
//从管道中读向buf中写
static int read_byte_unlock(my_pipe_t *ptr,char *src,size_t count){
  int i;
  struct my_pipe_st *p = ptr;
  for(i = 0; i < count; i++){
    *src = *(p->base + p->front);
...
static int write_byte_unlock(my_pipe_t *ptr,const char *src,size_t count){
  int i;
  struct my_pipe_st *p = ptr;
  for(i = 0; i < count; i++){
    *(p->base + p->rear) = *src;
...
int read_pipe(my_pipe_t *ptr,void *buf,size_t count){
  //读管道  要求管道里面有数据
...
int write_pipe(my_pipe_t *ptr,const void *buf,size_t count){
  int size;
  struct my_pipe_st *p = ptr;
  if(pthread_mutex_lock(&p->mut) != 0)
    return -1;
...
int destroy_pipe(my_pipe_t *ptr){
  //成功返回0，出错返回-1
  struct my_pipe_st *p = ptr;
  p->front = p->rear = 0;
  p->r_count = p->w_count = 0;
