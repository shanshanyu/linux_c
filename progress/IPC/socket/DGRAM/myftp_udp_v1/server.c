...
void daemonize(){
  ...
  if(fd0 != 0 && fd1 != 1 && fd2 != 2){
    syslog(LOG_INFO,"stdin stdout stderr redirect failed\n");
    exit(1);
  }
...
int file_check(const char *path){
  ...
  if(stat(path,&st) < 0){
    syslog(LOG_INFO,"%s file check failed\n",path);
    return -1;
  }
  ...
  if(fd < 0){
    syslog(LOG_INFO,"%s file check failed",path);
    return -1;
  }
  return fd;
}
...
void fsm_driver(fsm_udp_t *fsm){
  switch(fsm->state){
    case STATE_R_PATH:
      syslog(LOG_INFO,"start receive path.");
      ...
      if(fsm->read_count < 0){ //出错处理
        syslog(LOG_INFO,"recvfrom() faied.\n");
        fsm->state = STATE_Ex;
        break;
      }

      if(fsm->packet.u_path.mtype != MSG_PATH){ //判断收到的是否是 PATH 包
        syslog(LOG_INFO,"receive packet is not path.\n");
      ...
      if(fsm->fd < 0){
        syslog(LOG_INFO,"path is not a regular file.\n");
        fsm->state = STATE_R_PATH;
        break;
      }
      ...
      syslog(LOG_INFO,"end receive path.\n");


      syslog(LOG_INFO,"start send ack.\n");
      //发ack
    ...
    case STATE_S_DATA:
      ...
      if(fsm->read_count < 0){
        ...
        while(fsm->count < MAX_RETRY_TIMES){
          ...
          syslog(LOG_INFO,"start send eot.\n");
	  ...
	  if(sendto(fsm->sockfd,&fsm->packet.u_eot,sizeof(fsm->packet.u_eot),0,(void *)&fsm->raddr,sizeof(fsm->raddr)) < 0){
            syslog(LOG_INFO,"sendto(): send eot failed\n");
            fsm->state = STATE_Ex;
            break;
          }
          syslog(LOG_INFO,"end end eot.\n");
          ...
          if(fsm->ret < 0){
            ...
            syslog(LOG_INFO,"eot timeout,retry...");
            syslog(LOG_INFO,"eot retry local_ackno = %ld\n",fsm->local_ackno);
            fsm->count++;
            continue;
          }

          syslog(LOG_INFO,"start receive ack.\n");
          if(recvfrom(fsm->sockfd,&fsm->packet.u_ack,sizeof(fsm->packet.u_ack),0,(void *)&fsm->raddr,&fsm->raddrlen) < 0){
            syslog(LOG_INFO,"recvfrom(): receive ack failed\n");
            fsm->state = STATE_Ex;
            break;
          }
          ...
          fsm->recv_ackno++;
          fsm->state = STATE_R_PATH;
          syslog(LOG_INFO,"local_ackno = %ld.\n",fsm->local_ackno);
          syslog(LOG_INFO,"recv_ackno = %ld.\n",fsm->recv_ackno);
          syslog(LOG_INFO,"end receive ack.\n");
          syslog(LOG_INFO,"end send eot,change state to receive path.\n");
          break;
        }
        if(fsm->count == MAX_RETRY_TIMES){
          fsm->state = STATE_Ex;
          syslog(LOG_INFO,"exceed max retry times..\n");
        }
      }else{
        fsm->count = 0; //每次读之前初始化超时计数
        //发data
        while(fsm->count < MAX_RETRY_TIMES){
          ...
          syslog(LOG_INFO,"start send data.");
          fsm->packet.u_data.mtype = MSG_DATA;
          ...
          if(sendto(fsm->sockfd,&fsm->packet.u_data,sizeof(fsm->packet.u_data),0,(void *)&fsm->raddr,sizeof(fsm->raddr)) < 0){
            syslog(LOG_INFO,"sendto(): send data failed\n");
            fsm->state = STATE_Ex;
            break;
          }
          syslog(LOG_INFO,"end send data.\n");
          ...
          if(fsm->ret < 0){
            ...
            //超时
            syslog(LOG_INFO,"data timeout,retry...");
            syslog(LOG_INFO,"data retry local_ackno = %ld\n",fsm->local_ackno);
            fsm->count++;
            continue;
          }
        ...
        if(fsm->count == MAX_RETRY_TIMES){
          syslog(LOG_INFO,"exceed max retry times,state change to receive path.\n");
          fsm->state = STATE_R_PATH;
          break;
        }
      }
    ...
    case STATE_T:
      ...
      break;
    default:
      syslog(LOG_INFO,"fsm state abnormal\n");
      fsm->state = STATE_Ex;
  }
...
int main(){
  ...
  if(inet_pton(AF_INET,"172.19.229.90",&fsm.laddr.sin_addr) < 0){
    syslog(LOG_INFO,"inet_pton() failed\n");
    exit(1);
  }
  ...
  if(fsm.state == STATE_Ex){
    syslog(LOG_INFO,"process exit abnomal.\n");
  }else if(fsm.state == STATE_T){
    syslog(LOG_INFO,"process exit normal.\n");
  }
