...
void fsm_driver(fsm_udp_t *fsm){
  switch(fsm->state){
    case STATE_S_PATH:
      ...
      while(fsm->count < MAX_RETRY_TIMES){//重试
        ...
        if(fsm->packet.u_ack.mtype != MSG_ACK){//收包的包不是期望的包
          fprintf(stderr,"expect receive ack but receive packet is not ack.\n");
          fsm->count++;
          continue;
        }
      ...
      if(fsm->count == MAX_RETRY_TIMES){
        fprintf(stderr,"exceed max retry times.exit!\n");
        fsm->state = STATE_Ex;
      }
      break;
    ...
    case STATE_R_DATA:
      ...
      if(fsm->packet.u_data.ackno != (fsm->recv_ackno + 1)){
        fsm->state = STATE_R_DATA; //如果不是想要的包，继续收包
        fprintf(stderr,"packet is not wanted.\n");
        break;
      }
      fprintf(stderr,"end receive data.\n");
      /*if(fsm->packet.mtype != MSG_DATA || fsm->packet.mtype != MSG_EOT){
        fprintf(stderr,"receive packet is not data or eot packet.\n");
        fsm->state = STATE_R_DATA;
        break;
      }*/
