...
static void *thr_func(void *arg){
  ...
  while(1){
    ...
    for(i = 0; i < MYTBF_MAX; i++){
        ...
        //这里会触发条件变化
        err = pthread_cond_signal(&mytbf_arr[i]->cond);
        if(err != 0)
...
mytbf_t *mytbf_init(int cps,int burst){
  ...
  err = pthread_cond_init(&tbf->cond,NULL);
  if(err != 0)
...
int mytbf_fetchtoken(mytbf_t *tbf,int size){
  ...
  while(tbf->token <= 0){
    err = pthread_cond_wait(&tbf->cond,&tbf->b_lock);
...
int mytbf_returntoken(mytbf_t *tbf,int size){
  ...
  if(tbf->token > tbf->burst)
    tbf->token = tbf->burst;
  //这里会触发条件改变
  err = pthread_cond_signal(&tbf->cond);
  if(err != 0)
    err_print("pthread_cond_signal()",err);
...
int mytbf_destroy(mytbf_t *tbf){
  ...
  err = pthread_cond_destroy(&tbf->cond);
  if(err != 0)
    err_print("pthread_cond_destory()",err);
  free(tbf);
  return 0;
}
