 ...
 // fsm_node1->pos = 0; //写状态的位置
  ...
  while(fsm_node1->state != STATE_T || fsm_node2->state != STATE_T){
    ...
    if(fsm_node1->state == STATE_R)
      FD_SET(fsm_node1->fd_src,&rfds);
    if(fsm_node1->state == STATE_W)
      FD_SET(fsm_node1->fd_dst,&wfds);
    if(fsm_node2->state == STATE_R)
      FD_SET(fsm_node2->fd_src,&rfds);
    if(fsm_node2->state == STATE_W)
      FD_SET(fsm_node2->fd_dst,&wfds);  
    ...
    //监视
    err = select(max(fsm_node1->fd_src,fsm_node1->fd_dst)+1,&rfds,&wfds,NULL,NULL);
    if(err < 0){
      if(errno == EINTR)
        continue;
      else{
      ...
      //不会出现超时的情况，因为没有设定 timeout
    }else{
      //查看监视结果
      if(FD_ISSET(fsm_node1->fd_src,&rfds) || FD_ISSET(fsm_node1->fd_dst,&wfds))
        fsm_driver(fsm_node1);
      if(FD_ISSET(fsm_node2->fd_src,&rfds) || FD_ISSET(fsm_node2->fd_dst,&wfds))
        fsm_driver(fsm_node2);
    }
  }
