...
void relay(int fd_l,int fd_r){ //分别用 fd_l 表示左，fd_r 表示右
 ...
 // fsm_node1->pos = 0; //写状态的位置
  ...
  ev.events = 0;
  ev.data.fd = fd_l;
  if(epoll_ctl(epfd,EPOLL_CTL_ADD,fd_l,&ev) < 0){
    perror("epoll_ctl()");
    exit(1);
  }
  ...
  ev.events = 0;
  ev.data.fd = fd_r;
  if(epoll_ctl(epfd,EPOLL_CTL_ADD,fd_r,&ev) < 0){
    perror("epoll_ctl()");
    exit(1);
  }
  ...
  while(fsm_node1->state != STATE_T || fsm_node2->state != STATE_T){
    //这里有一个问题，会漏掉 state_ex->state_t,因为状态机到达 ex 态时，后续不会再去执行推状态机的操作了
    //布置监视任务
    /*ev.events = EPOLLIN;
    ev.data.fd = fd_l;
    if(fsm_node1->state == STATE_R)
      if(epoll_ctl(epfd,EPOLL_CTL_MOD,fd_l,&ev) < 0){
        perror("epoll_ctl()");
        exit(1);
      }
    ...
    ev.events = EPOLLOUT;
    ev.data.fd = fd_r;
    if(fsm_node1->state == STATE_W)
      if(epoll_ctl(epfd,EPOLL_CTL_MOD,fd_r,&ev) < 0){
        perror("epoll_ctl()");
        exit(1);
      }
    ...
    ev.events = EPOLLIN;
    ev.data.fd = fd_r;
    if(fsm_node2->state == STATE_R)
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_r,&ev);  //这里 fd_r 覆盖了上面的 EPOLLOUT，会有问题
   
    ev.events = EPOLLOUT;
    ev.data.fd = fd_l;
    if(fsm_node2->state == STATE_W)
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_l,&ev);  
    */   //这是一段有问题的代码
    ...
    if(fsm_node1->state == STATE_R){
      ev.events |= EPOLLIN;
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_l,&ev); //这条语句可以提取出来，状态机的判断只影响 events，epoll_ctl 可以等events确定好了调用一次
    }
    if(fsm_node2->state == STATE_W){
      ev.events |= EPOLLOUT;
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_l,&ev);
    ...
    if(fsm_node1->state == STATE_W){
      ev.events |= EPOLLOUT;
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_r,&ev);
    }
    if(fsm_node2->state == STATE_R){
      ev.events |= EPOLLIN;
      epoll_ctl(epfd,EPOLL_CTL_MOD,fd_r,&ev);
    ...
    //如果状态机的状态是 STATE_Ex,无条件推动状态机  如果是 STATE_T，就不推了
    if(fsm_node1->state == STATE_Ex)
      fsm_driver(fsm_node1);
    if(fsm_node2->state == STATE_Ex)
    ...
    while((epoll_wait(epfd,&ev,1,-1)) < 0){
      ...
      //查看监视结果
    if((ev.data.fd == fd_l && ev.events & EPOLLIN) || (ev.data.fd == fd_r && ev.events & EPOLLOUT)){
      fsm_driver(fsm_node1);
      printf("state 1\n");
    }
      
    if((ev.data.fd == fd_r && ev.events & EPOLLIN) || (ev.data.fd == fd_l && ev.events & EPOLLOUT)){
      fsm_driver(fsm_node2);
      printf("state 2\n");
    }
